...
export default class DataStore {
  ...
  addSeries(id, param) {
    ...
    const defaultSeries = {
      ...
      color: param.color || this.chartProp.colors[skey.length],
    ...
    if (!this.graphData[id] && this.chartProp.type !== 'pie') {
      this.graphData[id] = [];
    }
  }
  ...
  getSeriesExtends(defaultSeries, param) {
    const chartType = this.chartProp.type;
    const skey = Object.keys(this.seriesList);
    const horizontal = !!(this.chartProp.horizontal);
    ...
    const extSeries = {
      ...
      pointFill: param.pointFill || this.chartProp.colors[skey.length],
      lineWidth: param.lineWidth || 2,
      fill: param.fill || false,
      fillColor: param.fillColor || this.chartProp.colors[skey.length],
  ...
  addAxisData(axisType, data, axisIndex, dataIndex) {
    ...
    let axisOption;
    if (axisType === 'x') {
      axisOption = this.chartProp.xAxes[index];
    } else {
      axisOption = this.chartProp.yAxes[index];
    }
  ...
  createDummyAxis(axisType, axisIndex) {
    ...
    if (axisType === 'x') {
      option = this.chartProp.xAxes[axisIndex];
    } else {
      option = this.chartProp.yAxes[axisIndex];
    }

    for (let ix = 0; ix < keys.length; ix++) {
      maxLength = Math.max(maxLength, data[keys[ix]].length);
    }
  ...
  addGraphData(seriesId, axisData, graphData, dataIndex) {
    const isHorizontal = !!this.chartProp.horizontal;
    ...
    if (this.chartProp.bufferSize) {
      if (data.length >= this.chartProp.bufferSize) {
        data.shift();
        --index;
      }
    }
    ...
    if (axisType === 'x') {
      ...
      axisOption = this.chartProp.xAxes[axisIndex];
    } else {
      axisIndex = series.yAxisIndex;
      axisOption = this.chartProp.yAxes[axisIndex];
    }
